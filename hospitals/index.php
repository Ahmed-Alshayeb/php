...
switch ($method) {
    ...
    case 'POST':
        ...
        $stmt->execute();
        echo json_encode(["success" => true, "message" => "Hospital created"]);
        break;

    case 'PUT':
        // Logic to update a hospital
        parse_str(file_get_contents("php://input"), $put_vars);
        $id = intval($path_parts[2]);
        $name = validateInput($put_vars['name']);
        $location = validateInput($put_vars['location']);
        $stmt = $conn->prepare("UPDATE Hospitals SET name = ?, location = ? WHERE id = ?");
        $stmt->bind_param("ssi", $name, $location, $id);
        $stmt->execute();
        echo json_encode(["success" => true, "message" => "Hospital updated"]);
        break;
